...
\node[draw] (N) at (2,2) {Neuchâtel};
\draw[->,>=latex] (L) to[bend right] (N);
\draw[->,>=latex] (N) to[bend left] (C);
\draw[->,>=latex] (C) to[bend left] (L);
\end{tikzpicture}
...
\begin{tikzpicture}[scale=.8]
\node[draw,rectangle,rounded corners=3pt] (L) at (0,0) {Lausanne};
\node[draw,minimum height=1cm,dashed] (N) at (3,0) {Neuchâtel};
\node[draw,diamond,aspect=2.5] (C) at (6,0) {Coire};
\node[draw,ellipse] (G) at (9,0) {Genève};
\node[draw,circle,fill=gray!50] (Z) at (12,0) {Berne};
\end{tikzpicture}
...
\begin{tikzpicture}
 \draw[radial] (B) -- (L); \draw[radial] (B) -- (N);
 \draw[radial] (B) -- (C); \draw[radial] (B) -- (G);
 \draw[transversal] (N) -- (L);
 \draw[transversal] (L) -- (G);
 \draw[transversal] (G) -- (C);
\end{tikzpicture}
\end{lstlisting}
